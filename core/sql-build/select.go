...
func (s *SelectBuild) WhereAny(value interface{}, key string, rules ...Rule) SelectInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	s.whereAny(value, key, rule, GetWhereSetAnyValues)
	return s
}
...
func (s *SelectBuild) Or(value interface{}, key string, rules ...Rule) SelectInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	s.or(value, key, rule, GetWhereSetValues)
	return s
}
func (s *SelectBuild) OrAny(value interface{}, key string, rules ...Rule) SelectInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	s.orAny(value, key, rule, GetWhereSetAnyValues)
	return s
}
func (s *SelectBuild) Or_(value interface{}, key string, rules ...Rule) SelectInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	s.or_(value, key, rule, GetWhereSetValues)
	return s
}
