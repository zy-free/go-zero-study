...
func (d *DeleteBuild) WhereAny(value interface{}, key string, rules ...Rule) DeleteInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	d.whereAny(value, key, rule, GetWhereSetAnyValues)
	return d
}
...
func (d *DeleteBuild) Or(value interface{}, key string, rules ...Rule) DeleteInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	d.or(value, key, rule, GetWhereSetValues)
	return d
}

func (d *DeleteBuild) OrAny(value interface{}, key string, rules ...Rule) DeleteInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	d.orAny(value, key, rule, GetWhereSetAnyValues)
	return d
}

func (d *DeleteBuild) Or_(value interface{}, key string, rules ...Rule) DeleteInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	d.or_(value, key, rule, GetWhereSetValues)
	return d
}
...
func (d *DeleteBuild) String() (string, error) {
	...
	// orWhere
	var orWhere string
	if len(d.orWhereValues) > 0 {
		orWhere = strings.Join(d.orWhereValues, " or ")
	}
	//in notin
	var in, notin string
	...
	sql := "DELETE FROM " + d.tableName + lastWhere
