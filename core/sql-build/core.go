...
type BuildCore struct {
	...
	insertUpdateIgnoreColumns []string
...
func (b *BuildCore) whereAny(whereValue interface{}, key string, rule Rule, f func(values interface{}, rule Rule) (value string,
	err error)) {
	if b.err != nil {
		return
	}
	...
	if rule.StringValue == "" || value != strings.Join([]string{"'", "'"}, rule.StringValue) {
		if !strings.ContainsAny(key, ">=<") {
			key += " = "
		}
		b.whereValues = append(b.whereValues, key+value)
...
func (b *BuildCore) or(whereValue interface{}, key string, rule Rule, f func(values interface{}, rule Rule) (value string,
	err error)) {
	if b.err != nil {
		return
	}
	...
	if value != rule.StringValue && value != strings.Join([]string{"'", "'"}, rule.StringValue) {
		...
		b.orWhereValues = append(b.orWhereValues, key+value)
	}
}
func (b *BuildCore) orAny(whereValue interface{}, key string, rule Rule, f func(values interface{}, rule Rule) (value string,
	err error)) {
	if b.err != nil {
		return
	}
...
func (b *BuildCore) or_(whereValue interface{}, key string, rule Rule, f func(values interface{}, rule Rule) (value string,
	err error)) {
	if b.err != nil {
		return
	}
...
func (b *BuildCore) setAny(setValue interface{}, key string, rule Rule) {
	if b.err != nil {
		return
	}
	if key == "" {
	...
	value, err := GetWhereSetAnyValues(setValue, rule)
	if err != nil {
		b.err = err
		return
	}
...
func (b *BuildCore) value(ind reflect.Value, rule Rule, wg ...*sync.WaitGroup) {
	...
	for i, v := range b.insertTags {
		...
		if value == "DEFAULT" {
			b.insertUpdateIgnoreColumns = append(b.insertUpdateIgnoreColumns, b.insertColumns[i])
		}
	}
	if len(values) > 0 {
		//判断自增列为default
