...
> 注意1:insert方法因为需要在insert的方法上面加上db的tag,value和values方法不可以同时使用
...
```
首先,给`filed`加上`db`的tag,然后调用value,传入指针,同`select`一样,value方法也可以自定义规则传入使用.
sql打印:
```go
...
```go
type Tab struct {
	Id   int    `db:"id;auto;mycat:next value for MYCATSEQ_AGENT"`
	Name string `db:"name"`
	Age  int    `db:"age"`
}
...
|`Not found Insert Column`|没有找到insert的列名|tag db:"xxx"|
