...
func (u *UpdateBuild) SetAny(value interface{}, key string, rules ...Rule) UpdateInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	u.setAny(value, key, rule)
	return u
}
...
func (u *UpdateBuild) WhereAny(value interface{}, key string, rules ...Rule) UpdateInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	u.whereAny(value, key, rule, GetWhereSetAnyValues)
	return u
}
...
func (u *UpdateBuild) Or(value interface{}, key string, rules ...Rule) UpdateInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	u.or(value, key, rule, GetWhereSetValues)
	return u
}

func (u *UpdateBuild) OrAny(value interface{}, key string, rules ...Rule) UpdateInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	u.orAny(value, key, rule, GetWhereSetAnyValues)
	return u
}

func (u *UpdateBuild) Or_(value interface{}, key string, rules ...Rule) UpdateInf {
	var rule Rule
	if len(rules) > 0 {
		rule = rules[0]
	}
	u.or_(value, key, rule, GetWhereSetValues)
	return u
}
...
func (s *UpdateBuild) String() (string, error) {
	...
	// orWhere
	var orWhere string
	if len(s.orWhereValues) > 0 {
		orWhere = strings.Join(s.orWhereValues, " or ")
	}
	// in notin
	var in, notin string
	...
	if len(wheres) > 0 {
		...
		if orWhere != "" {
			lastWhere += " WHERE " + orWhere
		}
	} else {
		if orWhere != "" {
			lastWhere += " or " + orWhere
		}
	}

	sql := "UPDATE " + s.tableName + set + lastWhere
